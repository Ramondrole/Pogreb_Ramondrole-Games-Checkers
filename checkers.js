const checkersTranslations = {
    ru: {
        friendMode: "Против друга",
        botMode: "Против бота",
        whiteTurn: "Ход: Белые",
        blackTurn: "Ход: Чёрные",
    ...
    de: {
        friendMode: "Gegen Freund",
        botMode: "Gegen Bot",
        whiteTurn: "Zug: Weiß",
        blackTurn: "Zug: Schwarz",
...
let currentLang = localStorage.getItem('checkers_language') || 'ru';

function t(key, replacements = {}) {
    let text = checkersTranslations[currentLang]?.[key] || checkersTranslations.ru[key];
...
function updateCheckersUILanguage() {
    const elements = ['friendMode', 'botMode', 'newGameBtn'];
    ...
    if (gameOverFlag) {
        turnIndicator.textContent = t('gameOver');
    } else {
        turnIndicator.textContent = currentPlayer === 'white' ? t('whiteTurn') : t('blackTurn');
    }
}
...
function makeMove(move) {
    ...
    if (gameMode === 'bot' && currentPlayer === 'black' && !gameOverFlag) {
...
function drawBoard() {
    ...
    for (let r = 0; r < BOARD_SIZE; r++) {
        for (let c = 0; c < BOARD_SIZE; c++) {
            ...
            const isWhite = (piece === WHITE_MAN || piece === WHITE_KING);
            const isKingPiece = (piece === WHITE_KING || piece === BLACK_KING);
...
function changeLanguage(lang) {
    currentLang = lang;
    localStorage.setItem('checkers_language', lang);
    updateCheckersUILanguage();
    updateTurnDisplay();
    drawBoard();
}
