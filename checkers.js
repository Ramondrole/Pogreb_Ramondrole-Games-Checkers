(function() {
    ...
    function drawBoard() {
        ...
        if (selectedPiece) {
            ...
            ctx.fillStyle = 'rgba(179, 27, 27, 0.25)';
        ...
        for (let move of validMoves) {
            ...
            const x = toCol * size + size / 2;
            const y = toRow * size + size / 2;
            ctx.fillStyle = capture ? 'rgba(179, 27, 27, 0.6)' : 'rgba(179, 27, 27, 0.3)';
            ctx.beginPath();
        ...
        for (let r = 0; r < BOARD_SIZE; r++) {
            for (let c = 0; c < BOARD_SIZE; c++) {
                ...
                if (isWhite) {
                    ...
                    ctx.shadowColor = '#888899';
                } else {
                    ctx.fillStyle = '#b31b1b';
                    ctx.shadowColor = '#b31b1b';
                }
                ctx.shadowBlur = 15;
                ctx.beginPath();
                ...
                ctx.strokeStyle = isWhite ? '#888899' : '#661111';
                ctx.lineWidth = 1.5;
                ctx.stroke();

                if (isKingPiece) {
                    ctx.fillStyle = isWhite ? '#b31b1b' : '#ffffff';
                    ctx.shadowBlur = 10;
                    ctx.beginPath();
                    ctx.arc(x, y - radius / 2, radius / 2.5, 0, 2 * Math.PI);
                    ctx.fill();
                    ctx.shadowBlur = 0;
                }
            }
